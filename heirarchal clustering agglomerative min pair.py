...
def search_min_distance(clusters,data):
    min_distance = 90          # instead of arbitrary we can use max() to take in the largest number and compare to it
    for i in range(len(clusters)):
        for j in range(len(clusters)):
            if j > i:          # can use np.inf to insert infinity value to diagonal
                point_distances=np.ones((len(clusters[i]),len(clusters[j])))*100    #option of comparing as we go as alternative to matrix, can be faster
                ...
                # print(min_distance)
                if np.min(point_distances) < min_distance:     #use np.unravel_index(np.argmin(point_distances), point_distances.shape) to find i,j in a single line
                    min_row = i
                    min_col = j
                    min_distance = np.min(point_distances)
    print('normalized, minimal, minimal pair-wise distance (single linkage) is {}'.format(min_distance))
...
def agglomerative(data_initial,stop_condition,stop_number):    #the gunction name shhould be "fit"
    data_normalized = (data_initial-np.min(data_initial,0))/(np.max(data_initial,0)- np.min(data_initial,0))  #/feature_max       # w/ stats.zscore
...
stop_number = 5   #enter stop condition threshold

final = agglomerative(data_initial,stop_condition,stop_number)
print('\n')
score,falses = accuracy(final)     #only relevant for clusters=3
